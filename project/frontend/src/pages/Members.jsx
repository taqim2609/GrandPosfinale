...
export default function Members() {
  ...
  return (
    <div className="h-full overflow-y-auto p-6 lg:p-8" data-testid="members-page">
          ...
          <tbody>
            {items.map((m) => (
              <tr key={m.id} className="border-t" data-testid={`member-row-${m.id}`}>
                ...
                <td className="p-3 text-center text-[#047857] font-bold">Rp{Math.round((m.points || 0) * Number(biz?.member_redeem_per_point || 100)).toLocaleString("id-ID")}</td>
                <td className="p-3">
      ...
      <p className="text-xs text-[#8b87a8] mt-3 flex items-center gap-1.5"><Star size={13} /> Pelanggan mendapat <b>1 poin per Rp{Number(biz?.member_earn_per_rupiah || 10000).toLocaleString("id-ID")}</b> belanja saat transaksi memakai member. Tukar di kasir: <b>1 poin = Rp{Number(biz?.member_redeem_per_point || 100).toLocaleString("id-ID")}</b>. Nilai ini bisa diubah admin di Pengaturan → Aplikasi.</p>
