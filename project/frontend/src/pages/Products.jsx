      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label={form.weight_sale ? `Harga per ${form.weight_unit || "satuan berat"}` : "Harga Jual"}><input data-testid="prod-price" type="number" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} className="w-full h-11 rounded-xl border px-3 font-num" /></Field>
              {form.type === "vendor" ? (
            ...
            {(form.type === "makanan" || form.type === "minuman") && (
              <div className="rounded-xl border border-[#E4E4E7] p-3 space-y-2">
                <label className="flex items-center gap-2 text-sm font-bold cursor-pointer">
                  <input type="checkbox" data-testid="prod-weight-sale" checked={!!form.weight_sale} onChange={(e) => setForm({ ...form, weight_sale: e.target.checked })} className="h-4 w-4 accent-[#E63946]" />
                  Jual per berat (mis. ikan per ons)
                </label>
                {form.weight_sale && (
                  <div className="grid grid-cols-2 gap-2">
                    <Field label="Satuan Berat">
                      <select data-testid="prod-weight-unit" value={form.weight_unit || "ons"} onChange={(e) => setForm({ ...form, weight_unit: e.target.value })} className="w-full h-11 rounded-xl border px-3 bg-white">
                        <option value="ons">Ons</option><option value="gram">Gram</option><option value="kg">Kg</option><option value="porsi">Porsi</option><option value="pcs">Pcs</option>
                      </select>
                    </Field>
                    <div className="text-[11px] text-[#52525B] self-end pb-2">Harga di atas = harga per satuan. Contoh: 100.000/ons × 3 ons = 300.000.</div>
                  </div>
                )}
              </div>
            )}
