    ...
    <div className="flex h-screen overflow-hidden bg-[#F4F5F7]">
      ...
      <aside className={`fixed lg:static z-50 h-full w-[240px] shrink-0 text-white flex flex-col transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
        ...
        <nav className="flex-1 overflow-y-auto no-scrollbar py-3 px-3 space-y-1">
          {orderNav(NAV_ITEMS.filter((n) => canNav(n) && !(n.to === "/asisten-ai" && !aiVisible)), ui).map((n) => (
            <NavLink
              key={n.to}
              to={n.to}
              ...
              className={({ isActive }) =>
                `tap flex items-center gap-3 px-3 h-12 rounded-lg font-medium text-sm transition-colors ${
                  isActive ? "text-white shadow-lg shadow-black/20" : "text-white/80 hover:bg-white/15 hover:text-white"
                }`
              }
              style={({ isActive }) => (isActive ? { backgroundImage: "linear-gradient(90deg, var(--gak-p), var(--gak-a))" } : undefined)}
            >
              <n.icon size={18} /> {navLabel(n, ui)}
            </NavLink>
          ))}
        </nav>
