<?php
// Penerima laporan diagnostik dari server POS (tombol "Kirim ke VibeCoder").
// Menyimpan ke reports.jsonl di folder ini — dibaca oleh VibeCoder.
header('Content-Type: application/json');

$token = $_SERVER['HTTP_X_GAK_TOKEN'] ?? ($_GET['token'] ?? '');
if (!hash_equals('gak_rpt_7f3c9e1b', (string)$token)) {
