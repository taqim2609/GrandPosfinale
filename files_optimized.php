<?php
/**
 * Optimized Files List with Pagination - Week 1 & 2
 * Changes:
 * - Pagination (20 files per page)
 * - Debounced search on frontend
 * - File list caching
 */
require __DIR__ . '/lib_optimized.php';
...
$paginated_entries = array_slice($entries, $offset, $per_page);
...
$groups = array();
$total  = 0;
foreach ($paginated_entries as $e) {
    $total += $e['size'];
    $n     = $e['name'];
    $slash = strpos($n, '/');
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .pagination a, .pagination span {
    display: inline-block; padding: 8px 12px; margin: 0 4px; border-radius: 6px;
    text-decoration: none; font-size: 13px;
  }
  .pagination a { background: rgba(79,70,229,0.1); color: #4F46E5; cursor: pointer; }
  .pagination a:hover { background: rgba(79,70,229,0.2); }
  .pagination span.current { background: #4F46E5; color: #fff; font-weight: 700; }
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <div class="sub">
      ...
      Tidak perlu mengunduh arsip apa pun. <b style="color: #4F46E5;">✨ Sekarang dengan pagination!</b>
    </div>
    <?php if ($ver !== ''): ?>
  ...
  <div class="card">
    ...
    <?php if (!$paginated_entries): ?>
      <div class="empty">
        Folder <code class="k">project/</code> belum ada atau halaman ini kosong.
      </div>
    <?php endif; ?>
    ...
    <div class="pagination">
      ...
      <?php if ($page < $total_pages): ?>
        ...
        <a href="?page=<?= $total_pages ?>">Terakhir »</a>
      <?php else: ?>
        <span class="disabled">Selanjutnya ›</span>
        <span class="disabled">Terakhir »</span>
      <?php endif; ?>
    </div>
    <div class="foot">Halaman <?= $page ?> dari <?= $total_pages ?> (<?= $total_entries ?> berkas total)</div>
    <?php endif; ?>
  </div>
