...
function trySunmiPrinter(order, cfg, logoB64) {
  try {
    ...
    // Bukti pembatalan: penanda besar di paling atas supaya tidak tertukar dgn struk penjualan.
    if (order.void_note) {
      if (sp.setAlignment) sp.setAlignment(1);
      sp.printText("*** DIBATALKAN ***\n");
      if (sp.setTextSize) { try { sp.setTextSize(1.5); } catch (e) {} }
      sp.printText(`${order.void_note}\n`);
    ...
    payParts.forEach((pp) => {
      if (sp.printText) sp.printText(`${pp.payment_method_name}: ${rupiah(pp.amount)}\n`);
    });
    if (!(order.payment_splits && order.payment_splits.length) && order.change) sp.printText(`Kembali: ${rupiah(order.change)}\n`);
...
function browserPrint(order, cfg, logoB64) {
  ...
  <style>
    ...
    ${order.void_note ? `<div class="off" style="font-size:13px;padding:4px">*** DIBATALKAN ***</div><div class="c" style="font-weight:700;margin:2px 0">${esc(order.void_note)}</div>` : ""}
    <div class="hr"></div>
    ...
    <table>
      ...
      ${order.service_tax ? `<tr><td>Pajak Layanan</td><td class="r">+${rupiah(order.service_tax)}</td></tr>` : ""}
      <tr class="tot"><td>TOTAL</td><td class="r">${rupiah(order.total)}</td></tr>
      ${(order.payment_splits && order.payment_splits.length
        ? order.payment_splits.map((pp) => `<tr><td>${esc(pp.payment_method_name)}</td><td class="r">${rupiah(pp.amount)}</td></tr>`).join("")
        : order.payment_method_name ? `<tr><td>${esc(order.payment_method_name)}</td><td class="r">${rupiah(order.amount_paid || order.total)}</td></tr>` : "")}
      ${!(order.payment_splits && order.payment_splits.length) && order.change ? `<tr><td>Kembali</td><td class="r">${rupiah(order.change)}</td></tr>` : ""}
    </table>
    <div class="hr"></div>
