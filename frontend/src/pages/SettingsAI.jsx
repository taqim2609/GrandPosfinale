...
export default function SettingsAI() {
  ...
  const setAllGemini = async () => {
    if (!window.confirm("Set SEMUA fitur AI (deskripsi, gambar, analisis laporan, vision, asisten) ke provider Gemini? Model & key chenzk lama akan diabaikan/dibersihkan.")) return;
    setSavingKeys(true);
    try {
      for (const f of ["description", "image", "summary", "vision", "assistant"]) {
        // provider gemini + kosongkan base_url chenzk & model non-Gemini
        await api.put("/settings/ai", { feature: f, provider: "gemini", base_url: "", model: "" });
      }
      toast.success("Semua fitur AI kini memakai Gemini (model default Gemini)");
      load();
    } catch (e) { toast.error(apiError(e.response?.data?.detail)); }
    finally { setSavingKeys(false); }
  };
...
function FeatureCard({ featKey, data, onSaved }) {
  ...
  const pickProvider = (p) => {
    ...
    if (p === "gemini") setForm((f) => ({ ...f, base_url: "", model: "" })); // bersihkan sisa chenzk
  };
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const payload = { feature: featKey, provider };
      if (!isGemini) {
        payload.base_url = form.base_url.trim();
        if (form.model.trim()) payload.model = form.model.trim();
      } else {
        // Gemini: buang base_url chenzk + isi model hanya bila model Gemini valid
        payload.base_url = "";
        if ((form.model.trim() || "").toLowerCase().startsWith("gemini")) payload.model = form.model.trim();
      }
